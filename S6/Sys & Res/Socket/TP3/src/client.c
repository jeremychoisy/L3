...
int main(int argc, char *argv[]){
  ...
  char msg_in[50];      /* Message recu de "0" a "99" */
  char msg_out[20];    /* Message a envoyer "0" a "99" */
...
/* Recuperation de l'adresse IP */
    ...
    adr1.sin_port   = htons(50002); /* Meme port que sd0 : why not ? */
  ...
  /* 3) Boucle emission-reception
    ...
    /* a) Emission */
    for(;;){
        printf("\n\nEntrez le calcul à effectuer : (espace entre chaque entité)\n");
        scanf(" %[^\n]s",msg_out);
        fflush(stdin);
        printf("\n\nEnvoi ... ");
